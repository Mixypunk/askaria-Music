...
class SongTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    // Selector ciblé : ne rebuild que si c'est CE titre qui devient courant/non-courant
    final isCurrent = context.select<PlayerProvider, bool>(
        (p) => p.currentSong?.hash == song.hash);
    // Selector ciblé : ne rebuild que si l'état de DL de CE titre change
    final isDownloaded = context.select<DownloadsProvider, bool>(
        (d) => d.isDownloaded(song.hash));
